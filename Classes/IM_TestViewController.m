...
- (void)posterizeImageWithCompression {
    ...
    MagickWand * ori_magick_wand = NewMagickWand();
	NSData * dataObject = UIImagePNGRepresentation([UIImage imageNamed:@"Photo-1.png"]);//UIImageJPEGRepresentation([imageViewButton imageForState:UIControlStateNormal], 90);
	MagickBooleanType status;
	status = MagickReadImageBlob(magick_wand, [dataObject bytes], [dataObject length]);
    MagickReadImageBlob(ori_magick_wand, [dataObject bytes], [dataObject length]);
   ...
   MagickAdaptiveThresholdImage(magick_wand, 5 ,5, 10);
    ...
    MagickCompositeImage(ori_magick_wand , magick_wand , CopyOpacityCompositeOp ,0,0);
	...
	size_t my_size;
	unsigned char * my_image = MagickGetImageBlob(ori_magick_wand, &my_size);
	NSData * data = [[NSData alloc] initWithBytes:my_image length:my_size];
	free(my_image);
	ori_magick_wand = DestroyMagickWand(ori_magick_wand);
	MagickWandTerminus();
